...
export function derived(p) {
  ...
  for (const id of Object.values(p.equip)) {
    ...
    if (item.hpPenaltyPct) hpPenaltyPct += item.hpPenaltyPct;
  }
  const S=p.str, D=p.dex, I=p.int, V=p.vit, W=p.wis, L=p.luck;
  const hpBase = 30 + V*cfg.hp_per_vit + p.level*cfg.hp_per_level + b.hp;
  return {
    hpMax:  Math.floor(hpBase * (1 - hpPenaltyPct)),
