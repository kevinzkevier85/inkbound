...
import { makeNew, recompute, useItem, toggleEquip, classChange, availableClassChanges, derived, removeItem } from './core/playerSystem.js';
...
async function init() {
  ...
  await loadAll(new URL('../', import.meta.url).href.replace(/\/$/, ''));
  ...
  if (hasSave()) {
    ...
    if (p) {
      player = p;
      if (player.map === 'void_forest') player.map = 'graveyard'; // migrate old saves
      recompute(player);
      tick();
...
function openTravel() {
  ...
  body.innerHTML = `
    ...
    ${Object.entries(maps).map(([id, m]) => `
      <div class="class-card ${player.map === id ? 'current-map' : ''}" data-map="${id}">
        ...
        ${m.requiresKey ? ` <span style="color:var(--gold)">🔑 requires key</span>` : ''}
  ...
  body.querySelectorAll('[data-map]').forEach(card => {
    card.onclick = () => {
      ...
      // Void Dimension requires Key of the Void
      if (maps[id]?.requiresKey) {
        const keyId = maps[id].requiresKey;
        const keySlot = player.inv.find(s => s.id === keyId);
        if (!keySlot) {
          addLog(`You need a Key of the Void to enter ${maps[id].name}.`, 'system');
          modal.style.display = 'none';
          return;
        }
        removeItem(player, keyId);
        addLog('The key dissolves as the void tears open before you…', 'special');
      }
      player.map = id;
