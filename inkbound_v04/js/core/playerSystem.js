...
// Compute derived stats from base stats + config
export function derived(p) {
  ...
  for (const id of Object.values(p.equip)) {
    if (id && items[id]?.bonus) Object.entries(items[id].bonus).forEach(([k,v]) => { if (k in b) b[k]+=v; });
  }
...
// Apply class stat growth to player object (mutates)
...
// Sync hp/mp caps after stat changes
...
// Add EXP and handle level-ups (returns array of log messages)
export function gainExp(p, amount) {
  ...
  while (p.exp >= expForLevel(p.level)) {
    ...
    p.level += 1;
    const classes = getStore().classes;
    applyGrowth(p, classes[p.cls].growth);
    recompute(p, true);
...
// Returns log string or null if failed
